...
export default async function PrincipalMonitoringPage() {
  ...
  if (camerasError) {
    return (
      <EmptyState
        description={`Gagal memuat kamera NVR: ${camerasError.message}`}
        title="Daftar kamera tidak tersedia"
      />
    );
  }
  ...
  if (!cameras || cameras.length === 0) {
    return (
      <EmptyState
        description="Belum ada kamera CCTV yang diaktifkan atau dikonfigurasi oleh admin."
        title="Belum Ada Kamera Aktif"
      />
    );
  }
